...
多动症并不是代表着一种典型的症状个体，而是又多种行为症状表现而导致的一种疾病，所以定义多动症的界限虽然是固定的，但是它的行为准则的定义上确实模糊的，不具有非常典型的代表性。ADHD 是一种从童年开始，成长发展中不全面的一种表现形式。主要是在脑部前段发育不全，结构上比常规小。从心理诊断角度来说，一般表现出来的行为持续需要超过六个月以上才可以称作把不合理的行为称作症状。
...
ADHD 症状分为*两大类*: **注意力障碍** (Attention deficit) 和 **过动冲动 **(Hypotheticity)。
...
因为人的注意力是有容量限制的，而 ADHD 患者在这一方面的能力更加弱，所以可以借助奖励机制加上碎片化工作时间的方式来增加工作效率。这里要介绍 **番茄工作法** ，推荐一个应用叫做 Forest ，它可以在每次完成番茄钟之后为你种下一颗快乐的小树，番茄钟的长度可以自己设定，有注意力问题的人可以将时间设定得更短，然后在休息期间起来打一杯水，或者吃一颗糖，给自己奖励。最好别因为给自己奖励吃太多，不然容易变成一只温和的大白。
